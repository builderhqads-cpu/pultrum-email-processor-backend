...
describe('AiExtractionService', () => {
  ...
  it('analyzeEmail forwards attachmentsText to the router (Niek #10)', async () => {
    const configService: any = {
      get: jest.fn((key: string) => {
        if (key === 'AI_API_BASE_URL') return 'https://example.com';
        if (key === 'AI_EML_PROCESS_API_URL') return '/eml-process';
        return undefined;
      }),
    };
    ...
    await service.analyzeEmail('BASE64EML', {
      attachmentsText: 'Sheet: Orders\nrow1 data',
    });

    expect(captured.emlBase64).toBe('BASE64EML');
    expect(captured.attachmentsText).toBe('Sheet: Orders\nrow1 data');
  });

  it('analyzeEmail omits attachmentsText when it is blank', async () => {
    const configService: any = {
      get: jest.fn((key: string) => {
        if (key === 'AI_API_BASE_URL') return 'https://example.com';
        return undefined;
    ...
    await service.analyzeEmail('EML', { attachmentsText: '   ' });

    expect(captured.attachmentsText).toBeUndefined();
  });
});
